class Solution {
    public String subStrHash(String s, int power, int modulo, int k, int hashValue) {
		long[] powers = new long[k]; // Changed size to k
        for (int i = 0; i < k; i++) 
        ...
        while (index >= s.length() - k) {
            currentHashValue += ((s.charAt(index--) - 'a' + 1) % modulo * powers[powerIndex--]) % modulo; // Removed modulo from here
        }
    ...
    private long binaryExponentiation(long a, long b, long mod) {
        ...
        while (b > 0) {
            if (b % 2 == 1)
                result = (result * a) % mod; // Added modulo here
            a = (a * a) % mod; // Added modulo here
            b >>= 1;
        }
        return result;
    }
