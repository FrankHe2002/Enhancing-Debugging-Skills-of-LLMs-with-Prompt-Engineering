...
class Solution {
    ...
    private int maxValue(int[][] events, int index, int remainingEvents, int lastEventEndDay, int[][] dp) {
        ...
        int skip = maxValue(events, index + 1, remainingEvents, lastEventEndDay, dp); // skip
        int pick = maxValue(events, index + 1, remainingEvents - 1, events[index][1], dp) + events[index][2]; // pick

        return dp[remainingEvents][index] = Math.max(skip, pick);
    }
}
```