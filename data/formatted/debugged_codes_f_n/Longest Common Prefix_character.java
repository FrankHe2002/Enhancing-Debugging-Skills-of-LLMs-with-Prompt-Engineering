...
class Solution {
    ...
    private String findCommonPrefix(String str, int n) {
        String ans = "";
        TrieNode currRoot = root; // Fixed bug: create a new TrieNode variable to store the current root
        for (int i = 0; i < str.length(); i++) {
            if (currRoot.childs[str.charAt(i) - 'a'].frequency != n) { // Fixed bug: use the current root instead of the global root
                return ans;
            }
            ans += str.charAt(i);
            currRoot = currRoot.childs[str.charAt(i) - 'a']; // Fixed bug: use the current root instead of the global root
        }
        return ans;
    }
}