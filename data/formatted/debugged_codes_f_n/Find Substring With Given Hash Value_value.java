...
class Solution {
    public String subStrHash(String s, int power, int modulo, int k, int hashValue) {
        ...
        while (index >= s.length() - k) {
            ...
            currentHashValue = ((currentHashValue + (s.charAt(index--) - 'a' + 1) % modulo * powers[powerIndex--] % modulo) % modulo + modulo) % modulo;
        }
        ...
        for (int i = s.length() - k - 1; i >= 0; i--) {
            ...
            currentHashValue = ((currentHashValue - (((s.charAt(i + k) - 'a' + 1) * powers[k - 2]) % modulo) + modulo) % modulo + modulo) % modulo;
