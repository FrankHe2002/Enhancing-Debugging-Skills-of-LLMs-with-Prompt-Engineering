...
class Solution {
    ...
    public void dfs(int[] toppingCosts, int toppingIndex, int cost, int target) {
        ...
        dfs(toppingCosts, toppingIndex + 1, cost + toppingCosts[toppingIndex], target); // Fixed index bug
        dfs(toppingCosts, toppingIndex + 1, cost + 2 * toppingCosts[toppingIndex], target); // Fixed index bug
    }
}
```