...
class Solution {
    public int[] findEvenNumbers(int[] digits) {
        int[] map = new int[10]; // fixed size for frequency map
        ...
        for (int i = 100; i <= 999; i = i + 2) { // iterate from 100 to 999
            int num = i;
            int[] freq = new int[10];
            ...
            boolean res = findans(freq, map); // check if number can be formed using the available digits
            if (res) arr.add(i);
        }

        int[] ans = new int[arr.size()]; // convert ArrayList to array
        for (int i = 0; i < arr.size(); i++) { // copy elements to the array
            ans[i] = arr.get(i);
        }
    ...
    private boolean findans(int[] currentNum, int[] database) {
        for (int i = 0; i < 10; i++) { // compare the frequency of each digit
