...
class SortEnvelopes implements Comparator<int[]> {
    public int compare(int[] a, int[] b) {
        if (a[0] == b[0]) {
            // to ignore the duplicates, we are sorting such that, for the same width -> the element with
            // the largest height would be considered first, in this way all the other smaller heights would
            // be ignored
            return b[1] - a[1];
        } else {
            return a[0] - b[0];
