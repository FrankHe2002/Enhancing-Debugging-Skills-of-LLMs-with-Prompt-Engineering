...
class Solution {
    ...
    private long helper(int n, int dice, int prev, int[] rollMax, int runs) {
        ...
        long ans = 0;
        int[] temp = rollMax.clone(); // Fixed this line
        for (int i = 1; i <= 6; i++) {
            if (prev != 0 && i == prev && temp[i - 1] <= runs) // Fixed this line
                continue;
            if (i == prev)
                ans = (ans + helper(n, dice + 1, i, temp, runs + 1)) % mod; // Fixed this line
            else
                ans = (ans + helper(n, dice + 1, i, temp, 1)) % mod; // Fixed this line
        }
