...
class Solution {
    ...
    private boolean magic(
        ...
        int[] diagonal1 = new int[l];
        int[] diagonal2 = new int[l];

        // check both diagonals
        for (int k = 0; k < l; k++) {
            diagonal1[k] = grid[i + k][j + k];
            diagonal2[k] = grid[i + l - 1 - k][j + k];
        }

        if (target != Arrays.stream(diagonal1).sum() || target != Arrays.stream(diagonal2).sum()) {
            return false;
        }
