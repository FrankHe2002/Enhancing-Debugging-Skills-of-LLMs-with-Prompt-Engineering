...
class Solution {
    /*
     * dp[i][j] is the max length of Fibonacci series whose last two elements
     * are A[i] & A[j]
     * for any integer A[k] we need to find two numbers A[i] & A[j] such that
     * i < j < k and A[i] + A[j] == A[k; we can find such pairs in O(n) time
     * complexity.
     * if there exist i, j, k such that i < j < k and A[i] + A[j] == A[k, then
     * dp[k][j] = dp[i][j] + 1 (A[k], A[j] are last two elements of Fibonacci series)
     */
        ...
        for (int k = 2; k < n; k++) {
            ...
            while (i < j) {
                ...
                if (sum < 0) {
                    ...
                    // ith, jth, kth element are Fibonacci sequence
