...
class Solution {
        ...
        for (int i = 0; i < num.length(); ++ i) {
            ...
            for (int digit = 0; digit <= 9; ++ digit) {
                ...
                if (pqs.get(digit).size() != 0) {
                    ...
                    // Since few numbers already shifted to left, pos might be outdated
                    // we try to find out how many numbers already got shifted that were to the left of pos
                    int shift = seg.getCountLessThan(pos);
                    // (pos - shift) is the number of steps to move digit from pos to i
                    if (pos - shift <= k) {
                        k -= pos - shift;
    ...
    class SegmentTree {
        ...
        // Essentially, it counts the number of numbers less than num
