The bug in the given Java code is in the missing closing parenthesis in the for loop header. 

The line `for (int passengers : destinationToPassengers.values()` has a missing closing bracket `)`. It should be `for (int passengers : destinationToPassengers.values())`.

After the fix, the correct code will be:
