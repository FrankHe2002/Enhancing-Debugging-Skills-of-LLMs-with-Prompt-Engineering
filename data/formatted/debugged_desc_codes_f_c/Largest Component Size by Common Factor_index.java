...
class DisjointSet {
    ...
    public int find(int x) {
        while (x != root[x]) {
            root[x] = root[root[x]]; // Bug: Missed adding the rank optimization. Update the root of x.
            x = root[x];
        }
        return x;
    }
    ...
    public void union(int x, int y) {
        ...
        if (rank[rootX] > rank[rootY]) { // Bug: Incorrect indexing. Change rank[rootX + 1] to rank[rootX].
            root[rootY] = rootX;
        } else if (rank[rootX] < rank[rootY]) {
            root[rootX] = rootY;
        } else {
            root[rootY] = rootX;
            rank[rootX]++; // Bug: Increment rank of rootX, not rank[rootX].
        }
    }
}
```