...
Reasoning: The bug occurs in the following line:
```java
if (newRow >= 0 && newRow < rows && newCol >= 0 && newCol <= cols) {
```
The condition `newCol <= cols` should be `newCol < cols` to ensure the new column index is within bounds. The bug causes the program to access an index out of the `heights` array bounds.
