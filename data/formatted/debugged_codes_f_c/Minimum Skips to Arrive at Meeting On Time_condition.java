The bug in the code is in the line `dp[j] = Math.min(j == 0 ? INF : dp[j - 1] + dist[i], ceil(dp[j], speed) + dist[i]);`. The method `ceil()` should be called with `dp[j]` instead of `dp[j]`.
