Bug Type: Missing Generic Type and Null Check

Reasoning: In the provided code, `PriorityQueue<String> temp = new PriorityQueue();` is missing the generic type `<String>`. Additionally, the code does not check for null before calling the `poll()` method on `arrivals`, which can result in a NullPointerException.
