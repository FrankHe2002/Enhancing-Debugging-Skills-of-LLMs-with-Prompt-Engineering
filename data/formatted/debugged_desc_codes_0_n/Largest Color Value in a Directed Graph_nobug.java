...
class Solution {
    ...
    public int largestPathValue(String colors, int[][] edges) {
        ...
        boolean visiting[] = new boolean[colors.length()];
        boolean visited[] = new boolean[colors.length()];
        hasCycle = false;

        for (int i = 0; i < colors.length(); i++) {
            dfs(i, map, colors, visiting, visited);
    ...
    private Map<Character, Integer> dfs(int curr, Map<Integer, List<Integer>> map, String colors, boolean visiting[], boolean visited[]) {
        if (visiting[curr]) {
            hasCycle = true;
            return new HashMap<>();
        }
        ...
        if (list != null && !list.isEmpty()) {
            for (int i : list) {
                Map<Character, Integer> resMap = dfs(i, map, colors, visiting, visited);
