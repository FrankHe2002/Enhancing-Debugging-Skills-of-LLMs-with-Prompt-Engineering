...
class Solution {
    public String maximumNumber(String num, int[] change) {
        ...
        while (i < n) {
            ...
            if (change[digit] > digit) {
                ...
                while (i < n) {
                    ...
                    if (change[digit] <= digit) { // Fixed the comparison operator
                        break;
                    }
                    i += 1;
                }
        ...
        StringBuilder result = new StringBuilder(""); // Removed the extra quotes
        for (int j = 0; j < n; j++) { // Fixed the comparison operator
