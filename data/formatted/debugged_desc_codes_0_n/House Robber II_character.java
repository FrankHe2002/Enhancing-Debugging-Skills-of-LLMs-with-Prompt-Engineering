...
class Solution {
    public int rob(int[] nums) {
        ...
        int[] t = new int[nums.length]; // Fixed missing semicolon
        for (int i = 0; i < t.length; i++) {
            t[i] = -1; // Removed space between - and 1
        }
        int[] k = new int[nums.length];
        for (int i = 0; i < k.length; i++) {
            k[i] = -1; // Removed space between - and 1
        }
    ...
    static int helper(int[] nums, int i, int start, int[] t) {
        ...
        if (t[i] != -1) { // Removed space between - and 1
            return t[i];
        }
