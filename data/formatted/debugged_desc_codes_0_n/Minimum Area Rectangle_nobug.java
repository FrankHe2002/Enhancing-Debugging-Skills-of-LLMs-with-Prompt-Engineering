    ...
    public int minAreaRect(int[][] points) {
        ...
        for (int i = 0; i < points.length - 1; i++) {
            ...
            for (int j = i + 1; j < points.length; j++) {
                int x2 = points[j][0];
                int y2 = points[j][1];
                if (x1 == x2 || y1 == y2) { // Fixed condition for finding diagonal point, added curly braces
                    continue;
                }
                ...
                // it makes sense to check rectangle and override min (if these points form the correct rectangle)
