...
class Solution {
    public int minimumRounds(int[] tasks) {
        ...
        for (int i = 0; i < tasks.length; i++) { // Fixed loop condition
            taskMap.put(tasks[i], taskMap.getOrDefault(tasks[i], 0) + 1);
        }

        for (Map.Entry<Integer, Integer> entry : taskMap.entrySet()) {
            if (entry.getValue() == 1) {
                return -1; // Corrected space between '-' and 1
            }
            // try to take as many 3's as possible
            round += entry.getValue() / 3;

            /*
                We can have 1 or 2 tasks remaining. We're not supposed to take the task of count 1, but we can 'borrow' 1 from the previous
