        ...
        while (queue.size() > 0) {
            ...
            while (size-- > 0) {
                ...
                for (int k = 0; k < 4; k++) {
                    ...
                    if (rowDash < 0 || colDash < 0 || rowDash >= grid.length || colDash >= grid[0].length || visited[rowDash][colDash]) continue;
    ...
    private void dfs(int[][] grid, int i, int j, LinkedList<Pair> queue, boolean[][] visited) {
        ...
        for (int k = 0; k < 4; k++) {
            ...
            if (rowDash < 0 || colDash < 0 || rowDash >= grid.length || colDash >= grid[0].length ||
                    visited[rowDash][colDash] || grid[rowDash][colDash] == 0) continue;
