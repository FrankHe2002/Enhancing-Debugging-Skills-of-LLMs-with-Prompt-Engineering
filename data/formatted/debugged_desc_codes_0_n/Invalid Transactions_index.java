...
class Solution {
    public List<String> invalidTransactions(String[] transactions) {
        ...
        for (int i = 0; i < transactions.length; i++) {
            Transaction t = new Transaction(transactions[i], i); // fixed transactions[i] in creating Transaction object
            ...
            for (Transaction t : list) {
                if (t.isInvalidAmount()) invalid.add(transactions[t.id]);
                    ...
                    for (Transaction otherT : list) {
                        if (!t.name.equals(otherT.name) && t.isInvalidCity(otherT)) { // fixed to compare names and call isInvalidCity
