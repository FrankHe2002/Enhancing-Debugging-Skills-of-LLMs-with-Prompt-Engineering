...
class TrieNode {
    ...
    TrieNode() {
        ...
        this.frequency = 1; // fix: change frequency initialization to 1
    }
}
...
class Solution {
    ...
    private String findCommonPrefix(String str, int n) {
        String ans = "";
        TrieNode temp = root; // fix: create a temporary TrieNode
        for (int i = 0; i < str.length(); i++) {
            if (temp.childs[str.charAt(i) - 'a'].frequency != n) { // fix: use the temporary TrieNode here
                return ans;
            }
            ans += str.charAt(i);
            temp = temp.childs[str.charAt(i) - 'a']; // fix: use the temporary TrieNode here
        }
        return ans;
    }
}
