There is a bug in the code. The issue is with the line `minCost += cost[-1] + cost[index - 1];` inside the while loop. It should be `minCost += cost[index] + cost[index - 1];` as we want to access the elements at the index positions and not cost[-1].
