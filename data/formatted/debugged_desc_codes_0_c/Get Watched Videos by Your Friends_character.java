The code provided contains a bug in the for loop `for int i = 0; i < size; i++`. The syntax for the for loop is incorrect. The keyword `int` is not placed correctly.

To fix this bug, the code should be corrected in the for loop to `for (int i = 0; i < size; i++)`.
