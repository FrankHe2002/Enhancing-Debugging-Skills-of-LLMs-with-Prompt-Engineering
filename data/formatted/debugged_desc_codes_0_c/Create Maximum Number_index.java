The given code seems to have a bug in the `for` loop where it is iterating from 0 up to `k`, but it should be iterating only till `k - 1` because the index in the array is 0 based.
...
class Solution {
    public int[] maxNumber(int[] nums1, int[] nums2, int k) {
        ...
        int[] res = new int[k];
        for (int i = 0; i < k; ++ i) {
            res[i] = ans.charAt(i) - '0'; // fixed index
        }
        return res;
    }
