...
The bug in the code is in the `factory` method. There are two mistakes:
1. The loop condition in the `factory` method is incorrect. It should be `i <= n` instead of `i > n`.
2. The calculation for the `fact` array is incorrectly done in the `factory` method.
