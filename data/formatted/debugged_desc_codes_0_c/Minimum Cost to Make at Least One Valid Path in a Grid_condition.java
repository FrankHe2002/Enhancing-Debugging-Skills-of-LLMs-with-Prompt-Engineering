Bug: The `isValid` method in the `Solution` class contains a logical error. The condition `i < n && j > m` should be `i < n && j < m` in order to check if the given coordinates are within the boundaries of the grid.

Also, the `isValidDirection` method does not properly set the coordinates for the next cell based on the given direction.

I will first address the `isValid` method and then proceed with the `isValidDirection` method.
