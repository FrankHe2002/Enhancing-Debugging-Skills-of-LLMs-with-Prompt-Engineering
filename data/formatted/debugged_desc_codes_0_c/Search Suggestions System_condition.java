Bug:
- The while loop condition `while (p < pq.size() && p > 3)` is incorrect. It should be `while (p < 3 && !pri.isEmpty())`. 
- In the reduce method, the condition of `startsWith` is incorrect. It should be `if (s.startsWith(filter))`.
