Bug: The calculation of the currentHashValue inside the for loop seems to be incorrect. It looks like the ordering and the use of parentheses in the calculation is not correct.
...
class Solution {
    public String subStrHash(String s, int power, int modulo, int k, int hashValue) {
        ...
        for (int i = s.length() - k - 1; i >= 0; i--) {
            ...
            currentHashValue = (((currentHashValue - ((s.charAt(i + k) - 'a' + 1) * powers[k - 1] % modulo) + modulo) % modulo) * power % modulo + s.charAt(i) - 'a' + 1) % modulo;
