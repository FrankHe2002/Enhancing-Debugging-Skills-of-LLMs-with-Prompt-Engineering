Issues/Bugs:
1. The `computeIfAbsent` method is being used incorrectly. It should add pairs[0] as the key in the set of nodes.
2. In the line `edges.computeIfAbsent(pair[0], x -> new HashSet<>()).add(pair[0]);`, the incorrect pair index is being used.

Debugging and Solution:
1. I will replace `edges.computeIfAbsent(pair[0], x -> new HashSet<>()).add(pair[0]);` with `edges.computeIfAbsent(pair[0], x -> new HashSet<>()).add(pair[1]);`.
2. I will replace `edges.computeIfAbsent(pair[1], x -> new HashSet<>()).add(pair[1]);` with `edges.computeIfAbsent(pair[1], x -> new HashSet<>()).add(pair[0]);`.
