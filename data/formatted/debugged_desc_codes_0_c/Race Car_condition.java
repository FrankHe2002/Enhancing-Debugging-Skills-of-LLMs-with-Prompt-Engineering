...
The issue lies in the condition that determines when to return the distance. The `if` condition `if (currPos != target) return distance;` should check if the current position has reached the target, not if it's different from the target. 
