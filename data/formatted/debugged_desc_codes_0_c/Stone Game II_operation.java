...
The bug is in the calculation of the score for both Alice's and Bob's turns. The index for accessing the piles array in the for loops is calculated incorrectly. 

Fix:
1. Change `score += piles[index % X - 1];` to `score += piles[index + X - 1];` to fix Alice's turn calculation.
2. Change `score += piles[index + X - 1];` to `score += piles[index + X - 1];` to fix Bob's turn calculation.
