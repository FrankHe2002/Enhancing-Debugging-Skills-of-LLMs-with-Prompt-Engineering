The code may contain an issue with initializing the Union-Find data structure.

Upon initializing the `parent` array, there is a bug since the array is declared with a size of 1.
The bug can be fixed by initializing the `parent` array with the correct size `size`.
...
class Solution {
    ...
    class UnionFind {
        ...
        UnionFind(int size) {
            parent = new int[size];  // Fix: initialize with correct size
            rank = new int[size];
