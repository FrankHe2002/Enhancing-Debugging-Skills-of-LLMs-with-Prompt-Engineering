...
class Solution {
    ...
    private int palindrome(String word, int start, int end, int boundary, boolean isFound, int[][][] dp) {
        ...
        if (word.charAt(start) == word.charAt(end)) {
            return dp[start][end][isFound ? 0 : 1] = ((start == end ? 2 : 2) + palindrome(word, start + 1, end - 1, boundary, true, dp));
        } else {
            return dp[start][end][isFound ? 0 : 1] = Math.max(palindrome(word, start + 1, end, boundary, isFound, dp),
