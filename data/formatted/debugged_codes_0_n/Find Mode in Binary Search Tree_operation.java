...
class Solution {
    public int[] findMode(TreeNode root) {
        ...
        Map<Integer, Integer> map = new HashMap<>(); //we are taking map to count each and every value of the tree and the no of times they occurs in the tree
        Queue<TreeNode> qu = new LinkedList<>(); // to itereate over the tree
        List<Integer> list = new ArrayList<>(); //to save our result into a dynamic arraylist then will convert into static array for return it
        ...
        while (! qu.isEmpty()) {
            ...
            if (tmp.right != null)
                qu.add(tmp.right);    //the same thing of the above just this is for right node of respective poped out node
        }
