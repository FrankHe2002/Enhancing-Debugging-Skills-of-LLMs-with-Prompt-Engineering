...
class Solution {
    ...
    private boolean canPartition(int n, int k, int size, Set<Integer> current) {
        ...
        return addSet || canPartition(n - 1, k, size, new HashSet<>(current));
    }
    ...
    private void getAllSubsets(int n, int targetSum, Set<Integer> subsets, int[] nums, boolean lol) {
        ...
        subsets.add(n - 1);
        getAllSubsets(n - 1, targetSum - nums[n - 1], new HashSet<>(subsets), nums, true);
        subsets.remove(n - 1);
        getAllSubsets(n - 1, targetSum, new HashSet<>(subsets), nums, false);
    }
