...
class Solution {
    public static int fun(int index, List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        ...
        // Take Offer 
        int takeOffer = Integer.MAX_VALUE; // Changed value
        if (canTakeOffer(special.get(index), new ArrayList<>(needs))) {
            ...
            takeOffer = current_special.get(current_special.size() - 1) + fun(index - 1, price, special, new ArrayList<>(needs)); // Changed index
        }
    ...
    public static boolean canTakeOffer(List<Integer> current_special, List<Integer> needs) {
        ...
        for (int i = 0; i < current_special.size() - 1; i++) { // Changed starting index
