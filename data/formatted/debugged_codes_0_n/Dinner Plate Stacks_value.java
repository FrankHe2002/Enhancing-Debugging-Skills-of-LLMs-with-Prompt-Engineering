...
class DinnerPlates {
    ...
    public void push(int val) {
        while (!leftEmpty.isEmpty() && stack.get(leftEmpty.peek()).size() == cap) {
            leftEmpty.poll();
        }
        ...
        if (s.isEmpty()) {
            rightNonEmpty.offer(leftEmpty.peek());
        }
        s.push(val);
    }

    public int pop() {
        while (!rightNonEmpty.isEmpty() && stack.get(rightNonEmpty.peek()).isEmpty()) {
            rightNonEmpty.poll();
        }
        ...
        if (s.size() == cap) {
            leftEmpty.offer(rightNonEmpty.peek());
        }
        return s.pop();
    }
