...
class Solution {
    ...
    private int dfs(int cur, int where, Map<Character, TreeSet<Integer>> locMap, String key, int[][] memo) {
        ...
        int hcost = dfs(cur + 1, hi, locMap, key, memo) + (hi >= where ? hi - where : memo[cur][where] + hi);
        int lcost = dfs(cur + 1, lo, locMap, key, memo) + (lo < where ? where - lo : where + memo[cur][where] - lo);
