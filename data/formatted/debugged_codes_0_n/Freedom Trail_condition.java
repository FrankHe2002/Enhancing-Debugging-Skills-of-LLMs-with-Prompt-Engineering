...
class Solution {
    ...
    private int dfs(int cur, int where, Map<Character, TreeSet<Integer>> locMap, String key, int[][] memo) {
        ...
        int hcost = dfs(cur + 1, hi, locMap, key, memo) + Math.min(hi - where, memo[0].length - where + hi);
        int lcost = dfs(cur + 1, lo, locMap, key, memo) + Math.min(where - lo, where + memo[0].length - lo);
