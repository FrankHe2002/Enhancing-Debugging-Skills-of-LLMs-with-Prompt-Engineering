...
class Solution {
    ...
    public int shortestBridge(int[][] grid) {
        boolean[][] visited = new boolean[grid.length][grid[0].length]; // Fix typo
        LinkedList<Pair> queue = new LinkedList<>(); // Remove explicit type
        boolean found = false;
        for (int i = 0; i < grid.length && !found; i++) { // Remove extra space
            for (int j = 0; j < grid[0].length && !found; j++) { // Remove extra space
                if (grid[i][j] == 1) {
