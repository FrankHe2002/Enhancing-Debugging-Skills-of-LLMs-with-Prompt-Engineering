...
class Solution {
    public Node flatten(Node head) {
        ...
        while (!stack.isEmpty()) { // checking if the stack has still nodes in it.
            ...
            tail.next = curr; // pointing the previous node to the last node
            curr.prev = tail; // pointing previous pointer of the last node to the previous node.
            while (curr.next != null) { // traversing the last node's popped out of stack
                tail = curr;
                curr = curr.next;
            }
            tail = curr;
