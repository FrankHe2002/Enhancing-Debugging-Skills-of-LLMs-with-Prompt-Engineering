    ...
    public long numberOfWays(String s) {
        ...
        for (char ch : s.toCharArray()) {
            if (ch == '0') {
                ...
                if (zeroOne > 0 && one > 0) // Each of the previously formed 01 can form a triplet with the current 0 to form 010
                    tot += zeroOne * one;
            } else {
                one++;
                if (zero > 0)
                    zeroOne += zero; // Each of the previously found 0s can pair to form 01
                if (oneZero > 0 && zero > 0) // Each of the previously formed 10 can form 101
                    tot += oneZero * zero;
            }
        }
        return tot;
