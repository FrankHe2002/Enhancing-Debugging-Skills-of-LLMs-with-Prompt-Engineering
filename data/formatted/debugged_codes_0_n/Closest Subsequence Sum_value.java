...
class Solution {
    ...
    public int minAbsDifference(int[] nums, int goal) {
        ...
        for (int secondSetSum : second) {
            ...
            if (left < first.get(0)) { // all subset sums from first half are too big => Choose the smallest
                ans = Math.min(ans, Math.abs((left + secondSetSum) - goal));
                continue;
            }
            if (left > first.get(first.size() - 1)) { // all subset sums from first half are too small => Choose the largest
    ...
    void generate(int i, int end, int sum, List<Integer> listOfSubsetSums) {
        ...
        generate(i + 1, end, sum + arr[i], listOf