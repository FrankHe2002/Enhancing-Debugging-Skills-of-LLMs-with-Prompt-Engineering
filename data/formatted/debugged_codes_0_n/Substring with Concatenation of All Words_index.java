...
class Solution {
    public List<Integer> findSubstring(String s, String[] words) {
        ...
        int[] curWordId = new int[len]; // Change to int array
        ...
        for (int i = 0; i < (len - wordLen + 1); i++) {
            ...
            if (input.containsKey(cur)) {
                curWordId[i] = input.get(cur); // Fix curWordId[-1] to curWordId[i]
            } else {
                curWordId[i] = -1;
            }
        }
        ...
        for (int i = 0; i <= lastIndex; i++) { // Remove unnecessary space

            HashMap<Integer, Integer> winMap = new HashMap<>();
            for (int j = 0; j < windowLen && curWordId[i] != -1; j += wordLen) { // Fix -1 to -1L
