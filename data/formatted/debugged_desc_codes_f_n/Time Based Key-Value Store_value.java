...
class TimeMap {
    ...
    private String binarySearch(List<Entry> entries, int timestamp) {
        ...
        // Base cases - if timestamp is equal or greater, return the last value saved in map against this key, since that will have the largest timestamp
        if (entries.get(hi).timestamp <= timestamp) {
