...
class Solution {
    public int[] countPairs(int n, int[][] edges, int[] queries) {
        ...
        for (int j = 0, hi = n; j < queries.length; j++, hi = n) {
            for (int i = 0; i < n; i++) {
                while (hi > i && count[indexes[i]] + count[indexes[hi - 1]] > queries[j]) {
                    hi--;
                }
                ans[j] += n - hi;
                for (int adj : map.getOrDefault(indexes[i], Set.of())) {
                    int ttl = count[indexes[i]] + count[adj];
                    if (indexes[i] < adj && ttl > queries[j] && ttl - dupMap.getOrDefault(indexes[i], Collections.emptyMap()).getOrDefault(adj, 0) <= queries[j]) {
                        ans[j]--;
                    }
                }
            }
