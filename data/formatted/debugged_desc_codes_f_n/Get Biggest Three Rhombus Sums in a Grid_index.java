...
class Solution {
    public int[] getBiggestThree(int[][] grid) {
        ...
        if (maxThree[0] == 0) {
            if (maxThree[1] == 0) {
                return new int[] {maxThree[end - 1]};
            }
            return new int[] {maxThree[end - 1], maxThree[end - 2]};
        }
    ...
    void searchBigThree(int[][] grid, int[] maxThree, int length) {
        ...
        for (int start = 0; start < end; start++) {
            for (int start1 = length; start1 < end1; start1++) {
                if (start + start1 >= length && start + start1 <= grid.length - length && start1 + length <= grid[0].length - length) {
