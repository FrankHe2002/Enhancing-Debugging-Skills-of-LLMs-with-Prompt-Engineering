...
class Solution {
    public int leastBricks(List<List<Integer>> wall) {
        ...
        for (List<Integer> row : wall) //Iterating through each row
        {
            int edge_postion = 0; //Variable to store different edge postion

            for (int brick_no = 0; brick_no < row.size() - 1; brick_no++) //Iterating through each brick inside a row
            {
                int current_brick_length = row.get(brick_no); //Length of the current brick
                edge_postion += current_brick_length; //Next Edge Position = Previous Edge Position + Current Brick's Length
