...
class Solution {
    public int[][] rotateGrid(int[][] grid, int k) {
        ...
        for (int layerNo = 0; layerNo < noOfLayers; layerNo++) {
            int perimeter = (m - (2 * layerNo)) + (n - (2 * layerNo)) - 2; // Removed irrelevant calculations
    ...
    public void writeLayer(int[][] grid, int[] layer, int layerNo, int m, int n, int k) {
        ...
        for (int i = layerNo; i < n - layerNo; i++) {
            grid[layerNo][i] = layer[count % len];
            count++;
        }
        for (int i = layerNo + 1; i < m - layerNo; i++) {
            grid[i][n - layerNo] = layer[count % len];
            count++;
        }
        for (int i = n - layerNo; i > layerNo; i--) {
            grid[m - layerNo][i] = layer[count % len];
            count++;
        }
        for (int i = m - layerNo; i > layerNo; i--) {
            grid[i][layerNo] = layer[count % len];
            count++;
        }
    }
}
