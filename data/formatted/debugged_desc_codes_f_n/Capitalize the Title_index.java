...
class Solution {
    public String capitalizeTitle(String title) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            while (i < len && ch[i] != ' ') {
                ch[i] = Character.toLowerCase(ch[i]); // converting the character at ith index to lower case one by one
                ++i;
            }

            // if the word is of length greater than 2, then change the first character of the word to uppercase
            if (i - firstIndex > 2) {
