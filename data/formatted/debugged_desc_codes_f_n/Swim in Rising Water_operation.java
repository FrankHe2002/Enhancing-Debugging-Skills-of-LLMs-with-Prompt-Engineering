...
class Solution {
    public int swimInWater(int[][] grid) {
        ...
        while (left <= right) {
            int mid = left + (right - left) / 2; // Fixed the mid calculation formula
    ...
    boolean isValidCell(int[][] grid, int x, int y, int ans) {
        ...
        return ! (x < 0 || x >= grid.length || y < 0 || y >= grid[0].length || grid[x][y] <= ans); // Changed the elevation check to be less than or equal to
    }
}
```