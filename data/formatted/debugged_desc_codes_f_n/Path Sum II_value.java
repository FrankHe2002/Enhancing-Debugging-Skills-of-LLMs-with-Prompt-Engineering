...
class Solution {
    ...
    public void pathSum(TreeNode root, int targetSum, List<Integer> path, List<List<Integer>> ans) {
        ...
        {
            ans.add(new ArrayList(path));// we use new ArrayList because if we don't the original List is added which is mutable, if we add a copy that's not mutable.
        } else {
            pathSum(root.left, targetSum - root.val, new ArrayList(path), ans);
            pathSum(root.right, targetSum - root.val, new ArrayList(path), ans);
        }
        path.remove(path.size() - 1); //removal of redundant nodes. fix number from 0 to 1
    }
}
```