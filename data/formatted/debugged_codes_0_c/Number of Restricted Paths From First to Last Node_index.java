...
The bug is in the following line: `graph.putIfAbsent(e[newNode], new HashMap<>());`.

The variable `newNode` does not exist, and it should be `e[0]` instead. 

To fix the bug, we need to change `e[newNode]` to `e[0]` in that line. 
