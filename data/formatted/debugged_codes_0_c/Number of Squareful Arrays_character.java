The bug in the code is in the line where `set.add(nums[i])` is called. There is a missing closing parenthesis ')' after `nums[i]`.

To fix the bug, add the missing closing parenthesis in the line `set.add(nums[i])`.
