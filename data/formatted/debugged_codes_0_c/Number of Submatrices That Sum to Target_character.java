The provided code seems to have a typo on line 40. The expression `map.getdiff)` should be `map.get(diff)`.

To fix this bug, we need to change `map.getdiff)` to `map.get(diff)` on line 40.
