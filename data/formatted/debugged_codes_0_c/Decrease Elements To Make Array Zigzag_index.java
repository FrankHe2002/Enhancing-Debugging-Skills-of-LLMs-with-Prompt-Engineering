The bug in the provided code is in the calculation of the current element. 

In the line `int cur = arr[i - 1];`, the intended index of the current element is `i` instead of `i - 1`. 

To fix this bug, we need to change the line to `int cur = arr[i];`. 
