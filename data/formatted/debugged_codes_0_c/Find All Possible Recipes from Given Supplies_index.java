...
Bug: The for loop in the `findAllRecipes` method has an incorrect index calculation.

To fix this bug, we need to update the for loop index in line 10 from `recipes[Solution]` to `recipes[i]`.
...
By fixing this bug, we ensure that the correct index is used to access the `recipes` array, thus preventing any unexpected behavior or errors.