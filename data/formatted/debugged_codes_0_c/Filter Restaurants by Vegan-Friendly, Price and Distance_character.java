...
Explanation:
1. The bug occurs in the line `if (restaurant[3] <= maxPrice && restaurant[4] <= maxDistance {`.
2. The if statement is missing a closing parenthesis ")".

To fix the bug, add a closing parenthesis ")" after `maxDistance`.
