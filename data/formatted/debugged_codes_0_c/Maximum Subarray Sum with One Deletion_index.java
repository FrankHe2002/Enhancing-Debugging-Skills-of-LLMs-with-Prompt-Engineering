...
Bug explanation: In the nested for loop where we are finding the `leftPrefixSum`, there is a logical error in calculating the left prefix sum. Instead of subtracting `prefixSum[length]`, it should be subtracting `prefixSum[j]` to get the correct sum.
