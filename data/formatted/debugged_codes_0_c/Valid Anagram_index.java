There is a bug in the provided code. The bug is in the second for loop where it compares `haha1[0]` with `haha2[i]` instead of comparing `haha1[i]` with `haha2[i]`.

To fix this bug, we need to change `haha1[0]` to `haha1[i]` in the second for loop.
