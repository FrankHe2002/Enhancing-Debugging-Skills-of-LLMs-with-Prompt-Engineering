...
class Solution {
    ...
    public void dfs(int[] toppingCosts, int toppingIndex, int cost, int target) {
        ...
        dfs(toppingCosts, toppingIndex, cost + toppingCosts[toppingIndex], target);  // Fix bug here
        dfs(toppingCosts, toppingIndex, cost + 2 * toppingCosts[toppingIndex], target);  // Fix bug here
    }
}