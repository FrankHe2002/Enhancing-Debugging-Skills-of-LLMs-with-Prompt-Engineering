...
class Solution {
    public int largestVariance(String s) {
        ...
        int [] freq = new int[26];
        for(int i = 0 ; i < s.length() ; i++)         // FIX: missing parentheses for loop condition
            freq[(int)(s.charAt(i) - 'a')]++;
        
        int maxVariance = 0;
        for(int a = 0 ; a < 26 ; a++){
            for(int b = 0 ; b < 26 ; b++){
                int remainingA = freq[a];             // FIX: should use variable 'a' instead of 's'
                int remainingB = freq[b];             // FIX: should use variable 'b' instead of 's'
