class Solution {
    public TreeNode recoverFromPreorder(String traversal) {
        ...
        int bk = 0;
        for(int j = i; j <= traversal.length(); j++){  // Fix: changed 'i' to 'j'
            if(traversal.charAt(j-1) != '-' && traversal.charAt(j) == '-' && traversal.charAt(j+1) != '-')  // Fix: changed 'i' to 'j'
                bk = str.toString().length();
            else if(!(traversal.charAt(j-1) != '-' && traversal.charAt(j) == '-'))  // Fix: changed 'i' to 'j'
